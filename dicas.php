<?php
    include_once 'includes/header.php'
?>
<header>
<title>Dicas - Agro Malandrin</title>
</header>
 
<body id="dicas">
    <div id="conteiner">
        <table cellspacing="0" id="tableDicas1">
            <th id="titulo">Dicas Para Seu Pet</th>

            <tr>
                <td >
                    <ul>
                        <li><strong>Alimentação Balanceada:</strong> Escolha alimentos de qualidade e adequados à espécie e porte do seu pet;</li><br>

                        <li><strong>Atividade Física Regular:</strong> Proporcione exercícios diários para manter a saúde física e mental do animal;</li><br>

                        <li><strong>Visitas Regulares ao Veterinário:</strong> Mantenha check-ups periódicos para prevenir e detectar problemas de saúde precocemente;</li><br>

                        <li><strong>Higiene e Cuidados:</strong> Cuide da limpeza do ambiente e do pet, incluindo banhos, escovação e limpeza dos dentes;</li><br>

                        <li><strong>Atenção Emocional:</strong> Dedique tempo para interação e carinho, pois isso é essencial para o bem-estar emocional do seu pet.</li>
                    </ul>
                </td>
            </tr>
        </table>
        
        <table cellspacing="0" id="tableDicas2">
            <th id="titulo">Dicas para sua Agricultura</th>

            <tr>
                <td >
                    <ul>
                        <li><strong>Escolha de Culturas Adequadas: </strong>Selecione culturas que sejam adequadas ao clima, tipo de solo e disponibilidade de água da sua região. O planejamento também deve considerar a rotação de culturas para evitar a exaustão do solo e reduzir a incidência de pragas.</li><br>

                        <li><strong>Irrigação Inteligente:</strong> Utilize sistemas de irrigação eficientes, como gotejamento ou aspersão, para minimizar o desperdício de água. Monitorar o solo para irrigar apenas quando necessário ajuda a conservar recursos hídricos e a promover o crescimento saudável das plantas.</li><br>

                        <li><strong>Adubação Orgânica:</strong> Substitua ou complemente fertilizantes químicos com adubos orgânicos, como compostagem, para melhorar a fertilidade do solo e a saúde das plantas. Práticas como o uso de cobertura vegetal e plantio direto também ajudam a manter a integridade do solo.</li><br>

                        <li><strong>Manejo Integrado de Pragas (MIP):</strong> Combine métodos biológicos, físicos e químicos para o controle de pragas, minimizando o uso de pesticidas. Use insetos benéficos, plantas repelentes e armadilhas para manter o equilíbrio ecológico e proteger as culturas.
                        </li><br>
                        <li><Strong>Monitoramento Contínuo:</Strong> Use ferramentas tecnológicas para monitorar as condições das culturas, como sensores de solo e clima. A análise de dados em tempo real permite ajustar práticas agrícolas conforme necessário, garantindo a saúde das plantas e a eficiência da produção.</li>
                    </ul>
                </td>
            </tr>
        </table>
    </div>
</body>

<?php
include_once 'includes/footer.php';
?>

